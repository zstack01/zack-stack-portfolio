...
function FilmPortfolioWebsite() {
  ...
  const ProjectCard = ({ p, i, featured = false }) => {
    ...
    const cardHeight = featured ? "h-full" : "h-[360px]";
    const mediaHeight = featured ? "h-48" : "h-36";

    return (
      <motion.button
        key={`${featured ? "featured" : "project"}-${p.title}-${i}`}
        whileHover={featured ? { scale: 1.025, y: -4 } : undefined}
        onClick={() => setSelectedProject(p)}
        className="group block text-left"
      >
        <Card className={`relative flex ${cardHeight} flex-col overflow-hidden border ${featured ? "border-red-400/50" : "border-white/30"} bg-gradient-to-br ${getDepartmentAccent(p.role)} to-neutral-700 transition duration-300 hover:border-red-500/70 hover:shadow-2xl hover:shadow-red-900/35`}>
          {thumb ? (
            <PosterAwareImage project={p} src={thumb} className={`${mediaHeight} w-full shrink-0`} />
          ) : (
            <div className={`${mediaHeight} w-full shrink-0 border-b border-white/10 bg-black/10`} />
          )}
          <CardContent className="flex flex-1 flex-col justify-between p-5">
            <div>
              {p.status?.toLowerCase() === "in post" && (
                <div className="mb-3 flex flex-wrap items-center gap-2">
                  <span className="rounded-full border border-red-400/30 px-2.5 py-1 text-[10px] uppercase tracking-wider text-red-200/80">In Post</span>
                </div>
              )}
              <p className="mb-2 text-[11px] uppercase tracking-[0.22em] text-white/85">{p.type || "Project"} {p.year ? `• ${p.year}` : ""}</p>
              <h3 className={featured ? "line-clamp-2 text-xl font-black leading-tight text-white" : "line-clamp-2 text-lg font-black leading-tight text-white"}>{p.title}</h3>
              <p className="mt-2 line-clamp-2 text-sm text-gray-100">{p.role}</p>
              {p.company && <p className="mt-1 line-clamp-1 text-sm text-gray-100">{p.company}</p>}
            </div>
