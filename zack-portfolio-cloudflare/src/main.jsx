...
Forgive Me	Short Film	AIRED	https://www.imdb.com/title/tt30803278/?ref_=fn_t_9	1st Assistant Director	EchoBend	2023		false	
...
FTP x Vans	Commercial	AIRED	https://youtu.be/gNaU4Av09T8?si=Zido9ooblK00GLFd	2nd Assistant Director	Paperwork	2025		true	
...
function parseProjects(raw) {
  return raw
    ...
    .map((line) => {
      ...
      return {
        title: title.trim(), type: type.trim(), status: status.trim(), link: link.trim(), role: role.trim(),
        company: company.trim(), year: year.trim(), thumbnail: thumbnail.trim(),
        selected: String(selected).toLowerCase() === 'true', description: description.trim()
      };
    })
...
const roleGroups = ['All', 'Producer / LP / UPM / Production Manager', 'Production Coordinator', 'Production Assistant', '1st AD', '2nd AD', '2nd 2nd AD', 'AD PA', 'Camera', 'Electric', 'Art'];
const viewModes = ['All Credits', 'Linked Only'];

function normalizeRole(role) { return String(role || '').toLowerCase(); }
function getAllUrls(url) { return String(url || '').replaceAll('\n', ' ').replaceAll('\t', ' ').split(' ').map((i) => i.trim()).filter((i) => i.startsWith('http')); }
...
function getEmbedUrl(url) {
  ...
  try { const parsed = new URL(firstUrl); const host = parsed.hostname.replace('www.', ''); const parts = parsed.pathname.split('/').filter(Boolean); if (host.includes('vimeo.com')) return parts[0] ? `https://player.vimeo.com/video/${parts[0]}` : null; } catch { return null; }
  return null;
}
...
function getRoleGroup(role) {
  ...
  if (r === 'ad pa') return 'AD PA';
  if (r.includes('2nd 2nd') || r.includes('second second')) return '2nd 2nd AD';
  if (r.includes('2nd assistant') || r.includes('2nd ad')) return '2nd AD';
  if (r.includes('1st assistant') || r.includes('1st ad') || r.includes('assistant director')) return '1st AD';
  if (r.includes('camera')) return 'Camera';
  if (r.includes('electric')) return 'Electric';
  if (r.includes('art')) return 'Art';
  if (r.includes('producer') || r.includes('line producer') || r.includes('unit production manager') || r.includes('upm') || r.includes('production manager')) return 'Producer / LP / UPM / Production Manager';
...
function getDepartmentAccent(role) {
  ...
  if (['1st AD', '2nd AD', '2nd 2nd AD', 'AD PA'].includes(group)) return 'ad-accent';
  if (['Producer / LP / UPM / Production Manager', 'Production Coordinator', 'Production Assistant'].includes(group)) return 'prod-accent';
  return 'other-accent';
}
...
function shouldShowProject(project) {
  const status = String(project.status || '').toLowerCase(); const type = String(project.type || '').toLowerCase(); const title = String(project.title || '').toLowerCase(); const company = String(project.company || '').toLowerCase();
  const isInPost = status === 'in post'; const isShortFilm = type.includes('short film'); const isMexicali = title.includes('mexicali'); const isAlmostFriday = title.includes('almost friday') || company.includes('almost friday'); const isPanUp = company.includes('pan up productions'); const isLiveEvent = getProjectTypeGroup(project.type) === 'Live / Event';
  if (!hasThumbnailSource(project) && !isInPost && !isShortFilm && !isMexicali && !isAlmostFriday && !isPanUp && !isLiveEvent) return false;
  return true;
}
function getPrimaryRoleBadge(role) { return getRoleGroup(role); }
...
function sortProjects(a, b) { return Number(b.year || 0) - Number(a.year || 0); }
...
function ProjectCard({ project, onSelect, featured = false }) {
  ...
  return (
    <button className={`card project-card ${accent} ${featured ? 'featured-card' : ''}`} onClick={() => onSelect(project)}>
      {thumb ? <img src={thumb} alt="" className="thumb" /> : null}
      <div className="card-body">
        <div className="badges"><span>{getPrimaryRoleBadge(project.role)}</span>{project.status?.toLowerCase() === 'in post' && <span>In Post</span>}</div>
...
function App() {
  ...
  const filteredProjects = useMemo(() => projects.filter(shouldShowProject).filter((p) => {
    ...
    if (roleFilter !== 'All' && getRoleGroup(p.role) !== roleFilter) return false;
    return `${p.title} ${p.type} ${p.role} ${p.company} ${p.year}`.toLowerCase().includes(search.toLowerCase());
  }).sort(sortProjects), [projectTypeFilter, roleFilter, viewMode, search]);

  const featuredProjects = useMemo(() => {
    const selected = projects.filter((p) => p.selected && shouldShowProject(p));
    return (selected.length ? selected : projects.filter(shouldShowProject)).slice(0, 6);
  }, []);
  const crawlProjects = useMemo(() => filteredProjects.slice(0, 60), [filteredProjects]);
  ...
  return (
    <main className="site"><Header goToPage={goToPage} scrollToSection={scrollToSection} />
      {currentPage === 'home' && <>
        <section id="home" className="hero page-pad"><div className="hero-grid"><div><h1>Zack Stack</h1><p className="location">Los Angeles</p><p className="tagline">Producer & Assistant Director. Built for fast sets, clean execution, and controlled chaos.</p><div className="pill-row"><span>Available for travel</span><span>Commercials • Narrative • Music Videos</span></div><div className="cta-row"><button onClick={() => goToPage('work')}>View Work</button><button className="outline" onClick={() => scrollToSection('contact')}>Contact</button></div></div><div className="focus card"><p className="label">Focus</p><span>Producer</span><span>Assistant Director</span></div></div></section>
...
function Bio() { return <section id="bio" className="section"><div className="bio card"><p className="eyebrow">Bio</p><h2>Built for the plan. Ready for the pivot.</h2><p>Zack Stack is a Los Angeles-based Producer and Assistant Director working across commercials, narrative film, branded content, and music videos. He often steps into projects as a one-stop shop for production and AD work — building the plan, organizing the moving pieces, protecting the schedule, and keeping the day moving with calm urgency. Known for a hands-on work ethic and strong people skills, Zack is trusted in the space between client, talent, and crew: keeping communication clear, personalities supported, departments aligned, and the set moving without losing the room.</p></div></section>; }
function Contact() { return <section id="contact" className="section contact"><p className="eyebrow">Contact</p><h2>Let’s make the day.</h2><div className="contact-row"><a href="mailto:zstack01@gmail.com">Email</a><a href="tel:16262267919">Call</a><a href="https://instagram.com/ZackStack_" target="_blank" rel="noreferrer">Instagram</a><span>Los Angeles, CA</span></div></section>; }
...
function WorkSection(props) {
  const { currentPage, filteredProjects, crawlProjects, setSelectedProject, viewMode, setViewMode, search, setSearch, projectTypeFilter, setProjectTypeFilter, roleFilter, setRoleFilter } = props;
  return <section id="work" className={`section work ${currentPage === 'work' ? 'page-pad' : ''}`}><div className="section-head"><div><h2>All Work</h2><p>{filteredProjects.length} showing</p></div><div className="filter-row">{viewModes.map((m) => <button className={viewMode === m ? 'active' : ''} key={m} onClick={() => setViewMode(m)}>{m}</button>)}</div></div><input className="search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search title, role, company, year..." /><Filter label="Type of Project" items={projectTypeGroups} value={projectTypeFilter} setValue={(v) => { setProjectTypeFilter(v); setRoleFilter('All'); }} /><Filter label="Role" items={roleGroups} value={roleFilter} setValue={setRoleFilter} /><div className="crawl-wrap"><div className="fade left" /><div className="fade right" /><div className="crawl-track">{[...crawlProjects, ...crawlProjects, ...crawlProjects, ...crawlProjects].map((p, i) => <ProjectCard key={`${p.title}-${i}`} project={p} onSelect={setSelectedProject} />)}</div></div></section>;
}
